#!/usr/bin/env bash
# ops/runbooks/26-seed-solar-report-config.sh
#
# Idempotent seed: Solar Reinvestment Request template + daily schedule into PostgreSQL.
# Source HTML: db/seeds/solar-reinvestment-template.html
#
set -euo pipefail
...
TEMPLATE_HTML="${REPO_ROOT}/db/seeds/solar-reinvestment-template.html"
AUDIT_DIR="${REPO_ROOT}/data/audit/runbook-26"
TIMESTAMP="$(date -u +'%Y%m%dT%H%M%SZ')"

ENVIRONMENT="${ENVIRONMENT:-production}"
KISSFLOW_ACCOUNT_ID="${KISSFLOW_ACCOUNT_ID:-AcCMptlq60zH}"
APPLICATION_ID="${APPLICATION_ID:-Solar_Site_Expense_Governance_Syst_A00}"
PROCESS_ID="${PROCESS_ID:-Technician_Reimbursement__YTLM}"

TEMPLATE_ID="${SEED_TEMPLATE_ID:-a1a1a1a1-a1a1-4a1a-8a1a-a1a1a1a1a1a1}"
REPORT_DEF_ID="${SEED_REPORT_DEF_ID:-b2b2b2b2-b2b2-4b2b-8b2b-b2b2b2b2b2b2}"
REPORT_DEF_VERSION_ID="${SEED_REPORT_DEF_VERSION_ID:-c3c3c3c3-c3c3-4c3c-8c3c-c3c3c3c3c3c3}"
SCHEDULE_ID="${SEED_SCHEDULE_ID:-d4d4d4d4-d4d4-4d4d-8d4d-d4d4d4d4d4d4}"

TEMPLATE_NAME="Solar Reinvestment Request Report"
SCHEDULE_NAME="Daily Solar Reinvestment Request report"
CONTENT_REF="db/seeds/solar-reinvestment-template.html"
CRON_EXPRESSION="0 9 * * *"
...
log "Seeding Solar report config (${ENVIRONMENT} · ${APPLICATION_ID})"
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'subject', 'Kissflow - Solar Reinvestment Request Report',
    'legacy_template_id', 'tpl-refex-solar-reinvestment',
    'legacy_scheduler_id', 'sch-refex-solar-daily',
    'seed_runbook', '26-seed-solar-report-config'
  ),
  now()
)
...
cat > "${AUDIT_DIR}/runbook-26-${TIMESTAMP}.json" <<EOF
{
  "runbook": "26-seed-solar-report-config",
  "timestamp": "${TIMESTAMP}",
...
log "Audit: ${AUDIT_DIR}/runbook-26-${TIMESTAMP}.json"
